...
- **`src/quotescom`** – сам код приложения:  
...
4. **Задаём необходимые переменные окружения**
...
2. **Запускаем контейнер** (пример):
   ...
   - `-e SECRET_KEY=...` ‒ секретный ключ Flask 
   - `-e APP_DATA_DIR=/app_data` ‒ путь внутри контейнера, где хранится SQLite-файл.  
   - `-v $(pwd)/app_data:/app_data` ‒ монтируем локальную папку `app_data` в контейнер, чтобы база сохранялась между перезапусками.  
...
Список пакетов приведён в `requirements.txt`. Если кратко, приложение зависит от:
